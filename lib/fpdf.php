<?php 
    //Desarrollado por Juan Carlos Parra V 1.0
    //Fecha 03/02/2017
    ...
    class Pdf extends FPDF {
       ...
       public function Header(){
            //Imagen izquierda
            $this->Image('imagenes/facyt-mediano.gif', 10, 8, 12, 13);
            //Imagen derecha
            $this->Image('imagenes/LOGO-UC.png', 190, 8, 10,13);
            ...
            $this->Ln(3);
            $this->Cell(88);
            $this->Cell(30,10,'Inventario',0,'C');
            $this->Ln(11);///disminuir para que quede arriba
...
//            echo_pre($titcount);
			...
			$this->Ln();///quitar si lo quiero mas arriba
        }else{
            $this->Cell(30);
            $this->Cell(30,6,utf8_decode('Debe especificar el titulo del reporte como Array'),0,'C');
        }
		$this->SetFont('Arial','B','8');
		$this->Cell(12,10,utf8_decode('Total registros encontrados: '.sizeof($data)),0,'C');
        $this->SetTextColor('');
        $this->Ln();
        $this->SetFont('', '', 8);
        // Cabecera
...
//                    echo_pre($nuevo);
			...
			for ($i=0;$i<count($data);$i++){
				$this->Cell($this->widths[$i], 6, iconv('UTF-8', 'windows-1252',$data[$colum[($i)]]), '1', 0,'C');
			}
			}
    }else{
